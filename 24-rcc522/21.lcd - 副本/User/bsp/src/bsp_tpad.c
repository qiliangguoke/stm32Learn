...
uint8_t bsp_TpadScan(void)
{
	...
	if(rval>(tpad_default_val+tpad_default_val))
	{
		if(Keyen == 0)
		{
			res = 1;	//Keyen==0,有效
...
static void TPAD_TIM_Config(void)
{
	...
	TIM_ICInitStructure.TIM_Channel = TIMx_CAP_TIM_CHANNEL_x; 		/*配置输入捕获的通道，需要根据具体的GPIO来配置*/
	TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;		/*上升沿捕获*/
	TIM_ICInitStructure.TIM_ICSelection = TIM_ICSelection_DirectTI; /*输入通道和捕获通道的映射关系，这里选择直连*/
	TIM_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1; 			/*配置输入分频,不分频*/
	TIM_ICInitStructure.TIM_ICFilter = 0x03; 						/*IC2F=0011b配置输入滤波器8个定时器时钟周期滤波*/
