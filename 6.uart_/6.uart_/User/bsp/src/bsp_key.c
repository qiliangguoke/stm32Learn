...
/*
	该程序适用于WSNEP_V01开发板
	如果用于其它硬件，请修改GPIO定义和IsKeyDown1 - IsKeyDown6函数和 static void bsp_InitKeyHard(void)函数
...
static uint8_t IsKeyDown6(void)/*KEY1  KEY0组合键*/
{
	if((WKUP != WKUP_ACTIVE_LEVEL)&&(KEY0 == KEY0_ACTIVE_LEVEL)&&(KEY1 == KEY1_ACTIVE_LEVEL)&&(KEY2 != KEY2_ACTIVE_LEVEL))
		return 1;
	else
...
{
	s_tBtn[_ucKeyID].LongTime = _LongTime;		/*长按时间0 表示不检测长按事件*/
...
static void bsp_InitKeyVar(void)
{
	...
	for(i=0;i<KEY_COUNT;i++)
    {
		...
		s_tBtn[i].Count=KEY_FILTER_TIME/2;		/*计数器设置为滤波使劲按的一半*/
